...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomSensorHdle;                    /**< sensor handle */
  uint16_t  CustomTemperature_PressureHdle;                  /**< Temperature_Pressure handle */
...
/* Private variables ---------------------------------------------------------*/
uint16_t SizeTemperature_Pressure = 8;
...
#define COPY_SENSOR_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x23,0x23,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_TEMPERATURE_PRESSURE_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x23,0x24,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for sensor +
   *                                2 for Temperature_Pressure +
   *                                1 for Temperature_Pressure configuration descriptor +
   *                              = 4
   *
  ...
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             ...
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomSensorHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: sensor, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: sensor \n\r");
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomSensorHdle,
                          UUID_TYPE_128, &uuid,
                          SizeTemperature_Pressure,
                          CHAR_PROP_READ | CHAR_PROP_NOTIFY,
                          ATTR_PERMISSION_NONE,
                          GATT_NOTIFY_READ_REQ_AND_WAIT_FOR_APPL_RESP,
                          0x10,
                          CHAR_VALUE_LEN_VARIABLE,
                          &(CustomContext.CustomTemperature_PressureHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : TEMPERATURE_PRESSURE, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : TEMPERATURE_PRESSURE \n\r");
  }
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_TEMPERATURE_PRESSURE:
      ret = aci_gatt_update_char_value(CustomContext.CustomSensorHdle,
                                       CustomContext.CustomTemperature_PressureHdle,
                                       0, /* charValOffset */
                                       SizeTemperature_Pressure, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value TEMPERATURE_PRESSURE command, result : 0x%x \n\r", ret);
      }
      else
      {
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_TEMPERATURE_PRESSURE:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomSensorHdle, CustomContext.CustomTemperature_PressureHdle, SizeTemperature_Pressure, pPayload);
